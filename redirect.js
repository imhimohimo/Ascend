...
(async function main() {
  ...
  try {
    ...
    log(`MANUAL_OVERRIDE: ${manual || "(blank)"}`);
    ...
    error("CSV fetch/parse failed; attempting Overflow fallback.", err);
    const dest = await overflowFallback();
    return redirect(dest, "CSV error");
  }
})();
...
async function overflowFallback() {
  try {
    const rows = await fetchCsv(CSV_URL);
    const table = indexBySection(rows);
    const overflow = table[ROWS.OVERFLOW];
    if (overflow && overflow.PUBLIC_LINK) {
      log(`Overflow fallback URL: ${overflow.PUBLIC_LINK}`);
      return overflow.PUBLIC_LINK;
    }
  } catch (e) {
    warn("Overflow fallback failed", e);
  }
  return ABSOLUTE_DEFAULT_URL;
}
